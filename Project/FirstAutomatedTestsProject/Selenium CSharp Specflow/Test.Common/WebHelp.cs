...
namespace Selenium_CSharp_Specflow
{
    public static class WebHelp
    {
        ...
        public static void userNameElementInsert(String UserNameField, String userName)
        {
            try
            {
                WebHelp.Webdriver.FindElement(By.XPath(SearchPage.UserNameField)).SendKeys(userName);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
        ...
        public static void phoneElementInsert(String PhoneField, String phone)
        {
            try
            {
                WebHelp.Webdriver.FindElement(By.XPath(PhoneField)).SendKeys(phone);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
        ...
        public static void ITakeScreenShot()
        {
            ...
            ScreenShot.SaveAsFile(SetUp.resultsPath + SetUp.resultName + DateTime.Now.ToString("yyyy-MM-ddHHmmss") + ".jpeg", ScreenshotImageFormat.Jpeg);
        }
    }
}
